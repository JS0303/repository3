	...
	<div style="width: 98%; margin-left: 10px;">
		...
		<form name="detailForm" action="/listProduct.do?menu=${param.menu}"
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0"
				...
				/////////////////////// EL / JSTL 적용으로 주석 처리 //////////////////////// --%>
							<option value="0" ${ ! empty search.searchCondition && search.searchCondition ==0 ? "selected" : "" }>상품번호
							</option>
							<option value="1" ${ ! empty search.searchCondition && search.searchCondition ==1 ? "selected" : "" }>상품명
							</option>
							<option value="2" ${ ! empty search.searchCondition && search.searchCondition ==2 ? "selected" : "" }>상품가격
							</option>
					</select> <input type="text" name="searchKeyword" value="${! empty search.searchKeyword ? search.searchKeyword : "" }"
									...
									<c:when test="${product.prodTranCode eq '001'}">
										<td align="left">구매완료 <c:choose>
												<c:when test="${param.menu eq 'manage'}">
													<a
														href="updateTranCodeByProd.do?prodNo=${product.prodNo}&tranCode=002">배송하기</a>
												</c:when>
											</c:choose>
										</td>
									</c:when>
